  ...
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      if (el.src.startsWith('data:')) {
        ...
        imgOpts.path = el.src.startsWith('file://') ? el.src.replace('file://', '') : el.src;
        imgOpts.path = decodeURIComponent(imgOpts.path);
      }

      if (el.rounding) {
        imgOpts.rounding = true; // Enable rounding (makes it circular/oval)
        // PptxGenJS doesn't support custom border radius for images directly yet, 
        // but 'rounding: true' makes it an oval. 
        // For custom radius, we might need to use a shape fill, but let's stick to simple rounding for now.
      }
  ...
  return await page.evaluate(() => {
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'IMG') {
        ...
        if (rect.width > 0 && rect.height > 0) {
          const computed = window.getComputedStyle(el);
          const borderRadius = computed.borderRadius ? parseFloat(computed.borderRadius) : 0;
          ...
          elements.push({
            ...
            rounding: borderRadius > 0 // Add rounding flag if radius exists
          });
          processed.add(el);
          return;
        }
      ...
      if (decoration) {
        if (decoration.includes('underline')) baseStyle.underline = true;
        if (decoration.includes('line-through')) baseStyle.strike = true;
      }
